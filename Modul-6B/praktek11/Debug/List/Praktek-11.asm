...
;#include <mega8535.h>
	...
	#endif
;#include <delay.h>
;#define kirplus PORTD.1
;#define kirmin PORTD.2
;#define kanplus PORTD.6
;#define kanmin PORTD.3
;#define lpwm OCR1B
;#define rpwm OCR1A
...
; 0000 0021 {
	...
	.CSEG
; 0000 0022 kirplus=1;  kanplus=1;
; 0000 0023 kirmin=0;   kanmin=0;
; 0000 0024 }
;void mundur()
; 0000 0026 {
; 0000 0027 kirplus=0;  kanplus=0;
; 0000 0028 kirmin=1;   kanmin=1;
; 0000 0029 }
;void belok_kanan()
; 0000 002B {
; 0000 002C kirplus=1;  kanplus=0;
; 0000 002D kirmin=0;   kanmin=1;
; 0000 002E }
;void belok_kiri()
; 0000 0030 {
; 0000 0031 kirplus=0;  kanplus=1;
; 0000 0032 kirmin=1;   kanmin=0;
; 0000 0033 }
;void stop()
; 0000 0035 {
; 0000 0036 kirplus=0;  kanplus=0;
; 0000 0037 kirmin=0;   kanmin=0;
; 0000 0038 }
;void main(void)
; 0000 003A {
_main:
...
; 0000 0061 // Clock value: 250.000 kHz
; 0000 0062 // Mode: Fast PWM top=0x00FF
; 0000 0063 // OC1A output: Non-Inverted PWM
; 0000 0064 // OC1B output: Non-Inverted PWM
; 0000 0065 // Noise Canceler: Off
; 0000 0066 // Input Capture on Falling Edge
; 0000 0067 // Timer Period: 1.024 ms
; 0000 0068 // Output Pulse(s):
; 0000 0069 // OC1A Period: 1.024 ms Width: 0 us// OC1B Period: 1.024 ms Width: 0 us
...
; 0000 006E TCCR1A=(1<<COM1A1) | (0<<COM1A0) | (1<<COM1B1) | (0<<COM1B0) | (0<<WGM11) | (1<<WGM10);
	LDI  R30,LOW(161)
	OUT  0x2F,R30
; 0000 006F TCCR1B=(0<<ICNC1) | (0<<ICES1) | (0<<WGM13) | (1<<WGM12) | (0<<CS12) | (1<<CS11) | (1<<CS10);
	LDI  R30,LOW(11)
	OUT  0x2E,R30
...
; 0000 0084 TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (0<<TOIE1) | (0<<OCIE0) | (0<<TOIE0);
	OUT  0x39,R30
; 0000 0085 
...
; 0000 008A MCUCR=(0<<ISC11) | (0<<ISC10) | (0<<ISC01) | (0<<ISC00);
	OUT  0x35,R30
; 0000 008B MCUCSR=(0<<ISC2);
	OUT  0x34,R30
; 0000 008C 
; 0000 008D // USART initialization
; 0000 008E // USART disabled
; 0000 008F UCSRB=(0<<RXCIE) | (0<<TXCIE) | (0<<UDRIE) | (0<<RXEN) | (0<<TXEN) | (0<<UCSZ2) | (0<<RXB8) | (0<<TXB8);
	OUT  0xA,R30
; 0000 0090 
; 0000 0091 // Analog Comparator initialization
; 0000 0092 // Analog Comparator: Off
; 0000 0093 // The Analog Comparator's positive input is
; 0000 0094 // connected to the AIN0 pin
; 0000 0095 // The Analog Comparator's negative input is
; 0000 0096 // connected to the AIN1 pin
; 0000 0097 ACSR=(1<<ACD) | (0<<ACBG) | (0<<ACO) | (0<<ACI) | (0<<ACIE) | (0<<ACIC) | (0<<ACIS1) | (0<<ACIS0);
	LDI  R30,LOW(128)
	OUT  0x8,R30
; 0000 0098 SFIOR=(0<<ACME);
	LDI  R30,LOW(0)
	OUT  0x30,R30
; 0000 0099 
...
; 0000 00A9       kirplus=1;kirmin=0;
	SBI  0x12,1
	CBI  0x12,2
; 0000 00AA       lpwm=0;rpwm=0;
	LDI  R30,LOW(0)
	LDI  R31,HIGH(0)
	OUT  0x28+1,R31
	OUT  0x28,R30
...
; 0000 00AB       delay_ms(1000);
	...
	LDI  R27,HIGH(1000)
	RCALL _delay_ms
; 0000 00AC       kirplus=0;kirmin=1;
	CBI  0x12,1
	SBI  0x12,2
; 0000 00AD       lpwm=250;rpwm=250;
	LDI  R30,LOW(250)
	LDI  R31,HIGH(250)
	OUT  0x28+1,R31
	OUT  0x28,R30
